...
class ProcessMonitoringTickUseCase {
    ...
    func execute() async -> (newStatus: CursorInstanceStatus, newStatusMessage: String, shouldStopProcessingPID: Bool) {
        ...
        self.sessionLogger.log(
            level: .debug,
            message: "Executing monitoring tick for Cursor instance (PID: \(self.pid)). Current status: \(String(describing: self.currentInfo.status))",
            pid: self.pid
        )
        ...
        if let observationInfo = self.instanceStateManager.getPendingObservation(for: self.pid) {
            if Date()
                .timeIntervalSince(observationInfo.startTime) > Defaults[.postInterventionObservationWindowSeconds]
            {
                   ...
                   currentInterventionCount >= observationInfo.initialInterventionCountWhenObservationStarted
                {
                    ...
                    self.logger
                        .info(
                            "PID \(self.pid): Post-intervention observation window ended without positive activity. Consecutive failures: \(self.instanceStateManager.getConsecutiveRecoveryFailures(for: self.pid))"
                        )
                    self.sessionLogger.log(
                        level: .warning,
                        message: "Post-intervention observation window ended without positive activity. Consecutive failures incremented.",
                        pid: self.pid
                    )
                }
        ...
        if case .paused = self.currentInfo.status,
        ...
        case .generalError:
            ...
            self.logger
                .warning(
                    "PID \(self.pid) encountered general error (intervention type). Attempting generic stuck recovery."
                )
            ...
            self.logger
                .info("General error (stuck state) recovery attempt for PID \(self.pid) result: \(recoveryTried)")
            self.instanceStateManager.incrementAutomaticInterventions(for: self.pid)
            self.instanceStateManager.startPendingObservation(for: self.pid, initialInterventionCount: currentAttempts)
        ...
        case .unknown, .positiveWorkingState, .sidebarActivityDetected, .unrecoverableError, .manualPause,
            ...
            self.logger
                .debug(
                    "PID \(self.pid): Intervention type \(String(describing: interventionType.rawValue)) was not an actionable type for this switch. Current Status: \(String(describing: self.currentInfo.status))"
                )
            if self.currentInfo.status.isRecovering() || (self.currentInfo.status == .paused) {
                ...
                newStatus = .idle; newStatusMessage =
                    "Idle after error and \(String(describing: interventionType.rawValue))"
                self.instanceStateManager.resetAutomaticInterventions(for: self.pid)
                self.instanceStateManager.resetConsecutiveRecoveryFailures(for: self.pid)
                self.instanceStateManager.clearPendingObservation(for: self.pid)
            }
        ...
        if self.instanceStateManager
            .getAutomaticInterventions(for: self.pid) >= Defaults[.maxInterventionsBeforePause]
        {
            newStatus = .paused
            newStatusMessage = "Paused (Intervention Limit Reached)"
            self.logger
                .warning(
                    "PID \(self.pid) reached max interventions (\(Defaults[.maxInterventionsBeforePause])). Pausing automated interventions for this instance."
                )
            self.sessionLogger.log(
                level: .warning,
        ...
        if self.instanceStateManager
            .getConsecutiveRecoveryFailures(for: self.pid) >= Defaults[.maxConsecutiveRecoveryFailures]
        {
            newStatus = .unrecoverable(reason: "Max consecutive recovery failures reached.")
            newStatusMessage =
                "Unrecoverable: Max consecutive recovery failures reached (\(Defaults[.maxConsecutiveRecoveryFailures]))"
            self.logger
                .error(
                    "PID \(self.pid) has reached max consecutive recovery failures (\(Defaults[.maxConsecutiveRecoveryFailures])). Marking as unrecoverable."
                )
            self.sessionLogger.log(
                level: .error,
