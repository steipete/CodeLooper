...
public class CursorInterventionEngine: ObservableObject {
                                          ...
                                          runningApp _: NSRunningApplication) async -> InterventionType
    {
        ...
        if let generatingIndicatorLocator = await self.locatorManager.getLocator(
        ...
        if let errorMessageLocator = await self.locatorManager.getLocator(for: .errorMessagePopup, pid: pid) {
            ...
            if let axData = response.payload?.value {
                let textContent = self.getTextFromAXElement(AnyCodable(axData))
                if Self.errorIndicatingKeywords
                    .contains(where: { keyword in textContent.localizedCaseInsensitiveContains(keyword) })
                {
                    return .generalError
                }
        ...
        if Defaults[.monitorSidebarActivity] {
            if let sidebarLocator = await self.locatorManager.getLocator(for: .sidebarActivityArea, pid: pid) {
                ...
                let response = self.axorcist.handleQuery(command: queryCommand, maxDepth: 5)
                if let axData = response.payload?.value {
                    let sidebarTextRepresentation = self.getTextualRepresentation(
                        for: AnyCodable(axData),
                        depth: 0,
                        maxDepth: 1
                    ) // Wrap with AnyCodable
        ...
        if Defaults[.enableConnectionIssuesRecovery] {
            if let connectionErrorLocator = await self.locatorManager.getLocator(
                ...
                if let axData = response.payload?.value {
                    let textContent = self.getTextFromAXElement(AnyCodable(axData))
                    if Self.connectionIssueKeywords
                        .contains(where: { keyword in textContent.localizedCaseInsensitiveContains(keyword) })
                    {
    ...
    public func nudgeInstance(pid: pid_t, app _: NSRunningApplication) async -> Bool {
        self.logger
        ...
        guard let nudgeLocator = await self.locatorManager.getLocator(for: .mainInputField, pid: pid) else {
            self.logger
                .warning(
                    "PID \\(String(describing: pid)): Nudge failed - chat input locator (.mainInputField) not found."
                )
            self.sessionLogger.log(
                level: .warning,
                message: "Nudge failed - chat input locator (.mainInputField) not found.",
                pid: pid
            )
            return false
        }
        ...
        if focusResponse.error != nil {
            self.logger
                .warning(
                    "PID \\(String(describing: pid)): Failed to focus chat input before nudge: \\(String(describing: focusResponse.error?.message))"
                )
        }
        ...
        if setValueResponse.error == nil {
            ...
            self.instanceStateManager.startPendingObservation(
                for: pid,
                initialInterventionCount: self.instanceStateManager.getAutomaticInterventions(for: pid) - 1
            ) // -1 because we just incremented
            ...
            self.logger
                .warning(
                    "PID \\(String(describing: pid)): Failed to nudge (set value) on chat input: \\(String(describing: setValueResponse.error?.message))"
                )
            self.sessionLogger.log(
                level: .warning,
                message: "Failed to nudge (set value) on chat input: \\(String(describing: setValueResponse.error?.message))",
                pid: pid
            )
            return false
        }
    ...
    public func attemptConnectionRecovery(for pid: pid_t, runningApp _: NSRunningApplication) async -> Bool {
        ...
        guard let resumeButtonLocator = await self.locatorManager.getLocator(for: .resumeConnectionButton, pid: pid)
            ...
            self.logger
                .warning(
                    "PID \\(String(describing: pid)): Connection recovery failed - resume button locator (.resumeConnectionButton) not found."
                )
            self.sessionLogger.log(
                level: .warning,
                message: "Connection recovery: Resume button locator not found.",
                pid: pid
            )
            return false
        }
        ...
        if pressResponse.error == nil {
            ...
            self.logger
                .warning(
                    "PID \\(String(describing: pid)): Failed to press resume button: \\(String(describing: pressResponse.error?.message))"
                )
            self.sessionLogger.log(
                level: .warning,
                message: "Failed to press resume button: \\(String(describing: pressResponse.error?.message))",
                pid: pid
            )
            return false
        }
    ...
    public func attemptStuckStateRecovery(for pid: pid_t, runningApp _: NSRunningApplication) async -> Bool {
        ...
        self.logger
            .warning(
                "PID \\(String(describing: pid)): All stuck state recovery attempts (Force Stop/Resume, Stop Generating) failed."
            )
    ...
    static let positiveWorkKeywords = [
        ...
        "Indexing", "Indexed", "Synchronizing", "Synced", "Cloning", "Cloned", "Fetching", "Fetched", "Pushing",
        "Pushed",
        "Pulling", "Pulled", "Merging", "Merged", "Rebasing", "Rebased", "Committing", "Committed", "Deploying",
        "Deployed",
        ...
        "Opening", "Opened", "Closing", "Closed", "Updating", "Updated", "Installing", "Installed", "Uploading",
        "Uploaded",
        "Downloading", "Downloaded", "Processing", "Processed", "Compressing", "Compressed", "Decompressing",
        "Decompressed",
        "Waiting for", "Connected to", "Authenticated", "Authorized", "Verified", "Validated", "No issues found",
        "Succeeded",
    ...
    static let errorIndicatingKeywords = [
        ...
        "Timed out", "Refused", "Not found", "Unrecognized", "Unsupported", "Corrupt", "Aborted", "Canceled",
        "Terminated",
        "Disconnected", "Lost connection", "Build failed", "Test failed", "Syntax error", "Runtime error", "Crash",
        "Panic",
        "Fatal", "Segfault", "Segmentation fault", "Access violation", "Permission denied", "Could not", "Problem",
        "Issue",
    ...
    static let connectionIssueKeywords = [
        ...
        "language server timed out",
        "Copilot is encountering temporary",
        "Copilot failed to connect",
        "network error",
        "connection lost",
        "no internet",
        "offline",
    ...
    static let positiveActivityResetThreshold: TimeInterval =
        60.0 // Seconds of inactivity before resetting positive work state
    static let automatedInterventionCooldown: TimeInterval =
        30.0 // Seconds before another automated intervention can be tried for the same PID
    ...
    private func getTextualRepresentation(for axData: AnyCodable?, depth: Int, maxDepth: Int) -> String {
        ...
        if let element = data as? AXElement {
            ...
            if let title = element.attributes?[kAXTitleAttribute as String]?.value as? String,
               !title.isEmpty { representation += ", Title: \(title)" }
            if let value = element.attributes?[kAXValueAttribute as String]?.value as? String,
               !value.isEmpty { representation += ", Value: \(value)" }
            if let desc = element.attributes?[kAXDescriptionAttribute as String]?.value as? String,
               !desc.isEmpty { representation += ", Desc: \(desc)" }
            representation += "\n"
            ...
            if depth < maxDepth, let childrenData = element.attributes?[kAXChildrenAttribute as String]?.value {
                if let childrenArray = childrenData as? [Any?] {
                    // Children might be an array of AXElement or AnyCodable
                    for childData in childrenArray {
                        representation += getTextualRepresentation(
                            for: AnyCodable(childData),
                            depth: depth + 1,
                            maxDepth: maxDepth
                        )
                    }
