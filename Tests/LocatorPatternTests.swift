...
struct LocatorPatternTests {
    ...
    func locatorPatternRegistration() async throws {
    ...
    func locatorPatternMatching() async throws {
        let manager = LocatorManager.shared
        ...
        let matchesButton = await manager.matchesPattern(
            "submit-button",
            element: MockElement(role: "AXButton", title: "Submit")
        )
        let matchesTextField = await manager.matchesPattern(
            "username-field",
            element: MockElement(role: "AXTextField", identifier: "username-field")
        )
        ...
        let noMatch = await manager.matchesPattern(
            "submit-button",
            element: MockElement(role: "AXTextField", title: "Cancel")
        )
    ...
    func locatorPatternUpdates() async throws {
        let manager = LocatorManager.shared
    ...
    func dynamicLocatorDiscovery() async throws {
    ...
    func dynamicLocatorDiscoveryEmptyWindow() async throws {
        let discoverer = DynamicLocatorDiscoverer()
    ...
    func uIElementPatternValidation() async throws {
        // Test valid pattern
    ...
    func patternMatchingEdgeCases() async throws {
        let manager = LocatorManager.shared
        ...
        let exactMatch = await manager.matchesPattern(
            "case-test",
            element: MockElement(role: "AXButton", title: "Save")
        )
        ...
        let roleOnlyMatch = await manager.matchesPattern(
            "case-test",
            element: MockElement(role: "AXButton", title: "Any Title")
        )
    ...
    func concurrentPatternOperations() async throws {
        let manager = LocatorManager.shared
    ...
    func patternMatchingPerformance() async throws {
        let manager = LocatorManager.shared
