...
struct RuleExecutionTests {
    ...
    class MockJSHookService: JSHookService {
        ...
        override func getAllHookedWindowIds() -> [String] {
            return hookedWindowIds
        }
        
        override func sendCommand(_ command: [String: Any], to windowId: String) async throws -> String {
            if shouldThrowError {
    ...
    func testRuleExecutorGlobalMonitoring() async throws {
    ...
    func testDisabledRuleHandling() async throws {
        try withTemporaryDefaults {
            let executor = await RuleExecutor()
    ...
    func testStopAfter25LoopsRuleInitialization() async throws {
    ...
    func testStopAfter25LoopsRuleLimit() async throws {
        try withTemporaryDefaults {
            let rule = await StopAfter25LoopsRule()
    ...
    func testStopAfter25LoopsRuleExecution() async throws {
        try withTemporaryDefaults {
            let rule = await StopAfter25LoopsRule()
            let mockService = MockJSHookService()
    ...
    func testStopAfter25LoopsRuleNoAction() async throws {
        try withTemporaryDefaults {
            let rule = await StopAfter25LoopsRule()
            let mockService = MockJSHookService()
    ...
    func testStopAfter25LoopsRuleErrorHandling() async throws {
        try withTemporaryDefaults {
            let rule = await StopAfter25LoopsRule()
            let mockService = MockJSHookService()
    ...
    func testRuleCounterManagerSingleton() async throws {
    ...
    func testRuleCounterManagerTotalExecutions() async throws {
        let manager = await RuleCounterManager.shared
    ...
    func testRuleCounterManagerResetAll() async throws {
        let manager = await RuleCounterManager.shared
        let testRule1 = "TestRule1_\(UUID().uuidString)"
        let testRule2 = "TestRule2_\(UUID().uuidString)"
    ...
    func testRuleCounterManagerNotifications() async throws {
        let manager = await RuleCounterManager.shared
        let testRuleName = "TestRule_\(UUID().uuidString)"
    ...
    func testEndToEndRuleExecution() async throws {
        try withTemporaryDefaults {
            let executor = await RuleExecutor()
            let manager = await RuleCounterManager.shared
