...
func testInstanceCollectionOperations() async throws {
    ...
    let instances = [
        ...
        CursorInstanceInfo(app: mockApp2, status: .working(detail: "Active"), statusMessage: "Working", lastInterventionType: .connection),
        CursorInstanceInfo(app: mockApp3, status: .recovering(type: .stuck, attempt: 2), statusMessage: "Recovering", lastInterventionType: .stuck)
    ]
...
func testStatusTransitionValidation() async throws {
    ...
    let transitions = [
        ...
        (from: CursorInstanceStatus.working(detail: "Active"), to: CursorInstanceStatus.recovering(type: .connection, attempt: 1)),
        (from: CursorInstanceStatus.recovering(type: .connection, attempt: 1), to: CursorInstanceStatus.error(reason: "Failed")),
        (from: CursorInstanceStatus.error(reason: "Recoverable"), to: CursorInstanceStatus.recovering(type: .stuck, attempt: 1)),
        (from: CursorInstanceStatus.recovering(type: .stuck, attempt: 3), to: CursorInstanceStatus.unrecoverable(reason: "Max attempts")),
...
func testMemoryAndPerformance() async throws {
    ...
    let uniquePIDs = Set(instances.map { $0.pid })
...
private func createMockRunningApplication(pid: pid_t, bundleId: String?, name: String?) -> NSRunningApplication {
