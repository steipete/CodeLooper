...
enum CursorMonitorTestUtilities {
    ...
    static func createTestApp(
        ...
        for i in 0 ..< windows {
            await windowInfos.append(MonitoredWindowInfo(
                id: "window-\(id)-\(i)",
...
struct RequiresMonitoringActive: TestTrait {
    ...
    static func isEnabled(monitor: CursorMonitor) -> Bool {
        monitor.isMonitoringActivePublic
    }
}
...
struct CursorMonitorServiceTests {
    ...
    struct AppManagement {
        ...
        @MainActor func multipleAppsMonitoring() async throws {
            ...
            #expect(
                await monitor.monitoredApps.count == CursorMonitorTestData.testAppIds.count,
                "Should monitor all added apps"
            )

            for (index, expectedId) in CursorMonitorTestData.testAppIds.enumerated() {
                #expect(
                    await monitor.monitoredApps[index].pid == pid_t(expectedId),
                    "App \(index) should have correct PID"
                )
            }
        }
    }
    ...
    struct WindowManagement {
        ...
        func multipleWindowsPerApp(windowCount: Int) async throws {
            ...
            #expect(
                await monitor.monitoredApps.first?.windows.count == windowCount,
                "Should have \(windowCount) windows"
            )
        }
    }
...
extension CursorMonitorServiceTests {
    ...
    static func createTestAppInfo(
        ...
        await CursorMonitorTestUtilities.createTestApp(
            id: id,
            displayName: displayName,
            status: status
        )
