...
struct AttributeRowView: View {
    ...
    @State private var isSettable: Bool? = nil // Null until checked
    ...
    @State private var attributeUIType: AXAttributeValueType? = nil
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 2) {
                ...
                if attributeUIType == .axElement || attributeUIType == .arrayOfAXElements {
                    Button(attributeUIType == .arrayOfAXElements ? "Nav First" : "Navigate") {
                        Task {
                            ...
                            let axError = AXUIElementCopyAttributeValue(node.axElementRef, attributeKey as CFString, &elementRefCF)

                            if axError == .success, let valueRef = elementRefCF {
                                var refToNavigate: AXUIElement? = nil
                                if attributeUIType == .axElement {
                                    ...
                                    if CFGetTypeID(valueRef) == CFArrayGetTypeID() {
                                        ...
                                        if CFArrayGetCount(array) > 0 {
                                            ...
                                            axInfoLog("Attribute \(attributeKey) is an empty array of elements. Cannot navigate.")
                                        }
                                    } else {
                                ...
                                if let finalRefToNavigate = refToNavigate {
                                    viewModel.navigateToElementInTree(axElementRef: finalRefToNavigate, currentAppPID: node.pid)
                                    // If refToNavigate was for .axElement, it was the `valueRef` that was CFReleased by its own defer or outer scope.
                                    // If it was from an array, its lifetime is tied to the array `valueRef` which is released.
                                    // We should not release `finalRefToNavigate` here if it's just a pointer from the array.
                                    // This area is tricky with CF memory management.
                                    // Let's assume navigateToElementInTree does not take ownership, and the ref from array is valid for its call.
                                    // The original valueRef (single element or array) is released.
                                    if attributeUIType == .axElement, let singleElementValueRef = valueRef as! AXUIElement?, singleElementValueRef === finalRefToNavigate {
                                        // If it was a single element, it was already released by defer. This path is tricky.
                                        // Let's simplify: if it was a single AXUIElement, it needs release if it was the valueRef. If it was part of array, not here.
                                        // The defer { CFRelease(valueRef) } will handle the single element case correctly if valueRef was the direct AXUIElement.
                                        // This `if` block for .axElement should correctly release valueRef if refToNavigate was set from it.
                                    } 
                                } else if attributeUIType == .axElement {
                                     // If refToNavigate is nil, and type was .axElement, valueRef should have been released if it was non-nil by the outer defer.
                                     // This else branch is for cases where the cast/check failed inside .axElement case.
                                }
                            } else {
                                axWarningLog("Could not get value for attribute \(attributeKey) for navigation. Error: \(axError.rawValue)")
                            }
                        }
                    }
            ...
            .task(id: node.id.uuidString + attributeKey) { 
                let info = viewModel.fetchAttributeUIDisplayInfo(for: node, attributeKey: attributeKey, attributeValue: attributeValue)
