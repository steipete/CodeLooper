...
extension AXpectorViewModel {
    ...
    internal func calculateFullPath(for nodeID: AXPropertyNode.ID?, in tree: [AXPropertyNode], currentPathParts: [String] = []) -> String {
        guard let targetID = nodeID else { return "Error: No ID" }

        for node in tree {
            var pathPartsForCurrentNode = currentPathParts
            let nodeName = node.displayName.isEmpty ? "Unnamed" : node.displayName.replacingOccurrences(of: "/", with: ":") 
            ...
            if !node.children.isEmpty {
                let pathInChild = calculateFullPath(for: targetID, in: node.children, currentPathParts: pathPartsForCurrentNode)
    ...
    func expandNodeAndLoadChildren(_ node: AXPropertyNode) {
        if !node.hasChildrenAXProperty {
            axInfoLog("Node \(node.displayName) has no children to load according to AX property. Ensuring it is collapsed.")
        ...
        Task {
            ...
            axInfoLog("Successfully assigned \(childrenToAssign.count) children (after potential filtering) for node \(node.displayName).")
    ...
    internal func recursivelyFetchChildren(
        ...
        for childAX in directChildrenAX {
            ...
            if let t = childTitle, !t.isEmpty { childPathComponent += "[\"\(t.prefix(20))\"]" }
            else { childPathComponent += "[EL:\(String(describing: childAX.underlyingElement).suffix(8))]" } 
            let childFullPath = pathOfElementToFetchChildrenFor.isEmpty ? childPathComponent : "\(pathOfElementToFetchChildrenFor)/\(childPathComponent)"

            let grandChildrenPropertyNodes = await recursivelyFetchChildren(
                forElement: childAX,
                pid: pid,
                depthOfElementToFetchChildrenFor: depthOfElementToFetchChildrenFor + 1, // Grandchildren's parent is one level deeper
    ...
    internal func findNodeByAXElement(_ elementRef: AXUIElement, in nodes: [AXPropertyNode]) -> AXPropertyNode? {
        for node in nodes {
    ...
    internal func expandParents(for nodeID: AXPropertyNode.ID, in tree: [AXPropertyNode]) -> Bool { 
    ...
    internal func mapAXElementToNode(_ axElementFromCollectAll: Element, pid: pid_t, currentDepth: Int, parentPath: String) -> AXPropertyNode {
        ...
        let roleDesc = axElementFromCollectAll.attributes?[AXAttributeNames.kAXRoleDescriptionAttribute]?.value as? String
        ...
        if let t = title, !t.isEmpty { currentPathComponent += "[\"\(t.prefix(20))\"]" }
        else { currentPathComponent += "[EL:\(String(describing: axElementFromCollectAll.underlyingElement).suffix(8))]" } // Short unique-ish ID
        ...
        let node = AXPropertyNode(
            id: UUID(), axElementRef: axElementFromCollectAll.underlyingElement, pid: pid, role: role ?? "N/A", title: title ?? "",
