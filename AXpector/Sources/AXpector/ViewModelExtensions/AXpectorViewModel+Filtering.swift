...
// Alternatively, they could be nested inside the extension if preferred and Swift syntax allows for private/fileprivate struct access from extension methods.
// Making them internal here if they are used across the module, or fileprivate if only in this file + its extensions.

internal struct FilterCriterion { // Keep internal if AXpectorViewModel methods in other files might need it, or make fileprivate
    let key: String       
    let value: String     
    let isNegated: Bool   
    let isRegex: Bool     
...
internal struct GeneralTermCriterion { // Keep internal or make fileprivate
    let term: String
    let isNegated: Bool
    let isRegex: Bool     
}
...
extension AXpectorViewModel {
    internal func applyFilter() {
        ...
        if !generalTerms.isEmpty { axDebugLog("Parsed general terms: \(generalTerms.map { "(\($0.isNegated ? "NOT " : "")\($0.isRegex ? "regex:" : "")\($0.term))" })") }
    ...
    internal func parseFilterText(_ text: String) -> (criteria: [FilterCriterion], generalTerms: [GeneralTermCriterion]) {
        ...
        for component in components {
            ...
            if parts.count == 2, validKeys.contains(parts[0]) {
                ...
                let value = parts[1]
                // isRegex here correctly refers to whether the original component (after negation) started with regex: before the key.
                ...
                // If it wasn't a key:value, keyToTest is the term (with regex: already stripped if it was global for the term)
        ...
        for node in nodes {
            ...
            for criterion in criteria {
                ...
                switch criterion.key {
                    case "role": specificFieldMatches = match(text: node.role.lowercased(), pattern: targetValue, isRegex: criterion.isRegex)
                    case "title": specificFieldMatches = match(text: node.title.lowercased(), pattern: targetValue, isRegex: criterion.isRegex)
                    case "value": specificFieldMatches = match(text: node.value.lowercased(), pattern: targetValue, isRegex: criterion.isRegex)
                    case "desc": specificFieldMatches = match(text: node.descriptionText.lowercased(), pattern: targetValue, isRegex: criterion.isRegex)
                    case "path": specificFieldMatches = match(text: node.fullPath.lowercased(), pattern: targetValue, isRegex: criterion.isRegex)
                    case "id":
                        if let axIdentifier = node.attributes[AXAttributeNames.kAXIdentifierAttribute]?.value as? String {
                            specificFieldMatches = match(text: axIdentifier.lowercased(), pattern: targetValue, isRegex: criterion.isRegex)
            ...
            let negatedGeneralTerms = generalTerms.filter { $0.isNegated }
            ...
            if !positiveGeneralTerms.isEmpty {
                for termCrit in positiveGeneralTerms {
                    ...
                    if searchInDisplayName && match(text: node.displayName.lowercased(), pattern: termPattern, isRegex: termCrit.isRegex) { termMatchedInAtLeastOneField = true }
                    if !termMatchedInAtLeastOneField && searchInRole && match(text: node.role.lowercased(), pattern: termPattern, isRegex: termCrit.isRegex) { termMatchedInAtLeastOneField = true }
                    if !termMatchedInAtLeastOneField && searchInTitle && match(text: node.title.lowercased(), pattern: termPattern, isRegex: termCrit.isRegex) { termMatchedInAtLeastOneField = true }
                    if !termMatchedInAtLeastOneField && searchInValue && match(text: node.value.lowercased(), pattern: termPattern, isRegex: termCrit.isRegex) { termMatchedInAtLeastOneField = true }
                    if !termMatchedInAtLeastOneField && searchInDescription && match(text: node.descriptionText.lowercased(), pattern: termPattern, isRegex: termCrit.isRegex) { termMatchedInAtLeastOneField = true }
                    if !termMatchedInAtLeastOneField && searchInPath && match(text: node.fullPath.lowercased(), pattern: termPattern, isRegex: termCrit.isRegex) { termMatchedInAtLeastOneField = true }
            ...
            if !negatedGeneralTerms.isEmpty {
                for termCrit in negatedGeneralTerms {
                    let termPattern = termCrit.term
                    if searchInDisplayName && match(text: node.displayName.lowercased(), pattern: termPattern, isRegex: termCrit.isRegex) { matchesAnyNegatedGeneralTerm = true; break }
                    if searchInRole && match(text: node.role.lowercased(), pattern: termPattern, isRegex: termCrit.isRegex) { matchesAnyNegatedGeneralTerm = true; break }
                    if searchInTitle && match(text: node.title.lowercased(), pattern: termPattern, isRegex: termCrit.isRegex) { matchesAnyNegatedGeneralTerm = true; break }
                    if searchInValue && match(text: node.value.lowercased(), pattern: termPattern, isRegex: termCrit.isRegex) { matchesAnyNegatedGeneralTerm = true; break }
                    if searchInDescription && match(text: node.descriptionText.lowercased(), pattern: termPattern, isRegex: termCrit.isRegex) { matchesAnyNegatedGeneralTerm = true; break }
                    if searchInPath && match(text: node.fullPath.lowercased(), pattern: termPattern, isRegex: termCrit.isRegex) { matchesAnyNegatedGeneralTerm = true; break }
                }
            }
    ...
    internal func setupFilterDebouncer() {
        $filterText
    ...
    private func match(text: String, pattern: String, isRegex: Bool) -> Bool {
        if isRegex {
            do {
                ...
                axWarningLog("Invalid regex pattern '\(pattern)': \(error.localizedDescription). Treating as literal string contains search.")
