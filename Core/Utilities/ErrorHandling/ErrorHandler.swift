...
public final class ErrorHandler {
    ...
    private func shouldShowAlert(for error: AppError, context: ErrorContext) -> Bool {
        ...
        case .backgroundOperation:
            return error.category == .accessibility || error.category == .system
        case .general:
            return error.category != .monitoring && error.category != .network
        }
    }
    
    /// Get appropriate alert title for the error
    private func getAlertTitle(for error: AppError, context: ErrorContext) -> String {
        switch error.category {
        case .accessibility:
            return "Accessibility Permission Required"
        case .jsHook:
            return "JavaScript Hook Error"
        case .aiAnalysis:
    ...
    private func attemptErrorRecovery(_ error: AppError, context: ErrorContext) {
        ...
        Task { @MainActor in
            ...
            case .hookConnectionLost(let windowId):
