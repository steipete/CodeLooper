...
public final class ClaudeMonitorService: ObservableObject {
    ...
    public func syncWithUserDefaults() {
        ...
        logger
            .info(
                "Syncing Claude monitoring state: shouldMonitor=\(shouldMonitor), isCurrentlyMonitoring=\(isMonitoring)"
            )
            ...
            processQueue.async { [weak self] in
                ...
                for i in 0 ..< size / Int32(MemoryLayout<pid_t>.size) {
                    ...
                    if vinfoSize > 0 {
                        ...
                        logger
                            .info(
                                "Found Claude instance: PID=\(pid), workingDir=\(workingDir), status=\(status ?? "unknown")"
                            )
                    }
                }
                ...
                logger
                    .info(
                        "Scan complete: found \(nodeProcessCount) Node processes, \(newInstances.count) Claude instances"
                    )
                ...
                if ttyExists {
                    ...
                    if fd >= 0 {
                        ...
                        if bytesWritten > 0 {
                            logger
                                .info(
                                    "✅ Successfully updated terminal title for \(instance.folderName) (PID: \(instance.pid))"
                                )
    ...
    private func extractClaudeStatusForSpecificInstance(_ instance: ClaudeInstance) async -> String? {
        logger
            .info("Extracting Claude status for specific instance PID \(instance.pid) in \(instance.workingDirectory)")
    ...
    private nonisolated func extractClaudeStatusSync(ttyPath _: String, pid: Int32) -> String? {
    ...
    private nonisolated func getTerminalContentViaAccessibilityForInstance(_ instance: ClaudeInstance) -> String? {
        ...
        for terminalApp in terminalApps {
            if let content = getTerminalWindowContentForInstance(
                terminalPID: terminalApp.processIdentifier,
                instance: instance
            ) {
    ...
    private nonisolated func getTerminalContentViaAccessibility(pid _: Int32) -> String? {
        // Find terminal applications that might contain our Claude process
        let workspace = NSWorkspace.shared
        let runningApps = workspace.runningApplications
    ...
    private nonisolated func getTerminalWindowContentForInstance(terminalPID: pid_t,
                                                                 instance: ClaudeInstance) -> String?
    {
    ...
    private nonisolated func extractTextFromElement(_ element: AXUIElement, maxDepth: Int) -> String? {
        ...
        for attribute in textAttributes {
               ...
               !text.isEmpty
            {
                ...
                if text.contains("esc to interrupt") || text.contains("Reticulating") || text.contains("Thinking") {
                    logger
                        .debug("Found Claude status in \(role) element with \(attribute): \(String(text.prefix(200)))")
                    return text
                }
            }
        }
           ...
           let children = childrenRef as? [AXUIElement]
        {
            ...
            for child in children {
                ...
                {
                    if childRole.contains("ScrollArea") || childRole.contains("TextArea") || childRole
                        .contains("Text")
                    {
        ...
        do {
            ...
            for window in content.windows {
                ...
                if isTerminal, matchesInstance {
                    logger.info(
                        "Found matching terminal window: '\(windowTitle)' (\(appName)) for \(instance.folderName)"
                    )
    ...
    private nonisolated func captureWindowAndExtractTextModern(window: SCWindow) async -> String? {
        do {
            ...
            logger
                .debug(
                    "Captured window '\(window.title ?? "unknown")' using ScreenCaptureKit, size: \(image.width)x\(image.height)"
                )
            ...
            let nsImage = NSImage(
                cgImage: preprocessedImage,
                size: NSSize(width: preprocessedImage.width, height: preprocessedImage.height)
            )
    ...
    private nonisolated func parseClaudeStatusLine(_ text: String) -> String? {
        ...
        for pattern in patterns {
               ...
               )
            {
                ...
                let beforeText = String(cleanedText[..<cleanedText.index(
                ...
                let components = beforeText.components(separatedBy: "•")
                ...
                for component in components.reversed() {
                    ...
                    { // Add length constraint
                        ...
                        // Balance parentheses
                        let openCount = status.count { $0 == "(" }
                        let closeCount = status.count { $0 == ")" }
                ...
                if let lastParen = beforeText.lastIndex(of: "(") {
                       ...
                       status.contains("…") || status.contains("ing") || status.contains("tokens")
                    {
                        ...
                        logger
                            .debug(
                                "Rejected fallback status (too long or doesn't look like status): '\(String(status.prefix(100)))'"
                            )
                    }
                }
            }
