...
final class HTMLToMarkdownService {
    ...
    private func initializeOnBackgroundQueue() {
        queue.async { [weak self] in
            ...
            context.evaluateScript("console = { log: function() { consoleLog(Array.prototype.slice.call(arguments).join(' ')); } };")
    ...
    func convertToMarkdown(_ html: String) async throws -> String {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            queue.async { [weak self] in
                guard let self = self,
                      let domParser = self.domParser,
                      let turndownServiceClass = self.turndownServiceClass else {
...
extension HTMLToMarkdownService {
    enum MarkdownConversionError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .turndownNotLoaded:
                return "TurnDown.js library is not loaded"
            case .serviceUnavailable:
                return "Markdown conversion service is unavailable"
            case .conversionFailed:
                return "Failed to convert HTML to Markdown"
            case .domParserNotLoaded:
                return "LinkedOM DOMParser is not loaded"
            case .domParsingFailed:
                return "Failed to parse HTML with LinkedOM"
            }
        }
    }
