import SwiftUI
    ...
    final class RunLoopLocalEventMonitor {
        ...
        init(
            ...
            self
                .observer = CFRunLoopObserverCreateWithHandler(nil, CFRunLoopActivity.beforeSources.rawValue, true,
                                                               0)
                { _, _ in
                    ...
                    // Retrieve all events from the event queue to preserve their order (instead of using the `matching`
                    // parameter).
                    ...
                    // Iterate over the gathered events, instead of doing it directly in the `while` loop, to avoid
                    // potential infinite loops caused by re-retrieving undiscarded events.
    ...
    fileprivate extension SwiftUI.EventModifiers {
        // `.function` is deprecated, so we use the raw value.
        static let function_nonDeprecated = Self(rawValue: 64)
    }
