    ...
    public extension KeyboardShortcuts {
        /**
         A keyboard shortcut.
         */
        struct Shortcut: Hashable, Codable, Sendable {
            // MARK: Lifecycle
            ...
            public init?(event: NSEvent) {
                ...
                self.init(
                    ...
                    // Note: We could potentially support users specifying shortcuts with the Fn key, but I haven't
                    // found a reliable way to differentate when to display the Fn key and not. For example, with Fn+F1
                    // we only want to display F1, but with Fn+V, we want to display both. I cannot just specialize it
                    // for F keys as it applies to other keys too, like Fn+arrowup.
    ...
    private extension SpecialKey {
        var presentableDescription: String {
            ...
            case .keypad9:
                "9\u{20e3}"
            // There's "⌧“ 'X In A Rectangle Box' (U+2327), "☒" 'Ballot Box with X' (U+2612), "×" 'Multiplication Sign'
            // (U+00d7), "⨯" 'Vector or Cross Product' (U+2a2f), or a plain small x. All combined symbols appear bigger.
            case .keypadClear:
            ...
            case .keypadDivide:
                "/\u{20e3}"
            // "⏎" 'Return Symbol' (U+23CE) but "↩" 'Leftwards Arrow with Hook' (U+00d7) seems to be more common on
            // macOS.
            case .keypadEnter:
                "↩\u{20e3}" // The combined symbol appears bigger than the other combined 'keycaps'
            case .keypadEquals:
        ...
        var swiftUIKeyEquivalent: SwiftUI.KeyEquivalent? {
            switch self {
            case .return:
                .return
            case .delete:
        ...
        var appKitMenuItemKeyEquivalent: Character? {
            switch self {
            case .return:
                "↩"
            case .delete:
    ...
    extension KeyboardShortcuts.Shortcut {
        ...
        fileprivate func keyToCharacter() -> Character? {
            ...
            guard key.flatMap({ keyToSpecialKeyMapping[$0] }) == nil else {
                assertionFailure(
                    "Special keys should get special treatment and should not be translated using keyToCharacter()"
                )
                return nil
            }
                ...
                if let keyEquivalent = specialKey.swiftUIKeyEquivalent {
                    if #available(macOS 12.0, *) {
                        return KeyboardShortcut(
                            keyEquivalent,
                            modifiers: modifiers.toEventModifiers,
                            localization: .custom
                        )
                    } else {
